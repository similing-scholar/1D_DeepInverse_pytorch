...
def spectrum_deepinverse_encoder(spectrum_dataset, mask, encoder):
    """根据DeepInverse论文，将每一条光谱数据与mask相乘，再与mask.T相乘，得到长度不变的光谱编码。
    """
    if encoder == 'train':
        # 对spectrum_dataset每一行做变换
        for i in range(spectrum_dataset.shape[0]):
            observed_value = np.dot(mask, spectrum_dataset[i, :])  # 101 -> 42
            spectrum_dataset[i, :] = np.dot(mask.T, observed_value)  # 42 -> 101
...
class Basic1DDataset(Dataset):
    def __init__(self, mask_file, input_file, output_file=None, encoder='train'):
        ...
        self.input_data = spectrum_deepinverse_encoder(load_h5(input_file), self.mask, self.encoder)
...
def load_dataset(my_dataset, train_ratio, val_ratio, batch_size):
    ...
    # loader_args = dict(batch_size=batch_size, num_workers=os.cpu_count(), pin_memory=True)  # 多线程报错？？？？？？
